...
async function run() {
  try {
    const sites = await makeApiCall('GET', '/configurations/latest');
    const siteIds = sites.handlers['broken-backlinks'].enabled.sites;

    for (const siteId of siteIds) {
      // eslint-disable-next-line no-await-in-loop
      const site = await makeApiCall('GET', `/sites/${siteId}`);
